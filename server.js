...
const isValidPassword = (password) => {
  // Requires: 1 Letter, 1 Number, 1 Symbol, min 6 chars
...
app.post('/api/auth/register', async (req, res) => {
  try {
    ...
    if (!isValidPassword(password)) return res.status(400).json({ success: false, message: 'Password must have numbers, letters, and a symbol (min 6 characters)' });
...
app.get('/api/doctors', async (req, res) => {
  try {
    ...
    if (spec) query.specialization = { $regex: new RegExp('^' + spec + '$', 'i') };

    const docs = await User.find(query).select('-password');
